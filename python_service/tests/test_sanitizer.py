...
from services.proto import sanitizer_pb2, sanitizer_pb2_grpc
...
class TestSanitizerService:
    ...
    def test_sanitize_document_success(self):
        ...
        with patch("main_grpc_server.sanitize_file") as mock_sanitize:
            mock_sanitize.return_value = "sanitized content"

            request = sanitizer_pb2.SanitizeRequest(
                document_data=b"test document", document_type="application/pdf"
            )
    ...
    def test_sanitize_document_value_error(self):
        ...
        with patch("main_grpc_server.sanitize_file") as mock_sanitize:
            ...
            request = sanitizer_pb2.SanitizeRequest(
                document_data=b"test document", document_type="invalid"
            )
            ...
            context.set_details.assert_called_once_with("Invalid document type")
