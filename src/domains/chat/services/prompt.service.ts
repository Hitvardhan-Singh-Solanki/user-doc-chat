...
`.trim();
    ...
    const initialTokens = this.tokenizer ? this.tokenizer.countTokens(prompt) : Math.ceil(prompt.length / 4); // fallback approximation
    if (initialTokens > finalConfig.maxLength!) {
      ...
      if (finalConfig.truncateStrategy === 'truncate-history') {
        const targetTokens = Math.max(
          0,
          (this.tokenizer ? this.tokenizer.countTokens(sanitizedHistory) : Math.ceil(sanitizedHistory.length / 4)) - overflow - buffer,
        );
        ...
        const targetTokens = Math.max(
          0,
          (this.tokenizer ? this.tokenizer.countTokens(sanitizedContext) : Math.ceil(sanitizedContext.length / 4)) - overflow - buffer,
        );
        truncatedText = this.truncateByTokens(
          sanitizedContext,
          targetTokens,
      ...
      const finalTokens = this.tokenizer ? this.tokenizer.countTokens(prompt) : Math.ceil(prompt.length / 4);
      ...
      this.logger.info(
        {
          ...
          tokens: this.tokenizer ? this.tokenizer.countTokens(prompt) : Math.ceil(prompt.length / 4),
...
`.trim();
    ...
    const initialTokens = this.tokenizer ? this.tokenizer.countTokens(prompt) : Math.ceil(prompt.length / 4);
    if (initialTokens > finalConfig.maxLength!) {
      // FIX: Corrected log order
      this.logger.warn(
        { initialTokens, maxLength: finalConfig.maxLength },
      ...
      const targetLen = Math.max(
        0,
        (this.tokenizer ? this.tokenizer.countTokens(content) : Math.ceil(content.length / 4)) - overflow - buffer,
      );
...
`.trim();
  ...
  private truncateByTokens(
    ...
    for (const s of sentences.reverse()) {
      const t = this.tokenizer ? this.tokenizer.countTokens(s) : Math.ceil(s.length / 4);
      if (used + t <= maxTokens) {
        kept.push(s);
        used += t;
      }
