...
describe('PromptService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new PromptService(new SimpleTokenizerAdapter());
    // Wait for tokenizer initialization
  ...
  describe('mainPrompt', () => {
    ...
    it('should handle truncation when prompt is too long', async () => {
      ...
      const mockTokenizer = {
        encode: vi
          .fn()
          .mockReturnValueOnce(new Array(2000)) // First call: high count
          .mockReturnValue(new Array(800)), // Subsequent calls: lower count
      };
    ...
    it('should throw error when truncation strategy is "error"', async () => {
      ...
      const mockTokenizer = {
        encode: vi.fn().mockReturnValue(new Array(2000)),
      };
